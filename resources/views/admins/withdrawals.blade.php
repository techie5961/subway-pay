       ...
       @else
           @foreach ($trx as $data)
            ...
            <div class="row g-10">
               ...
               <div class="status {{ $data->status == 'success' ? 'green' : ($data->status == 'pending' ? 'gold' : 'red') }} m-left-auto">{{ $data->status ?? '' }}</div>
            </div>
            <hr>
            ...
            @switch($data->type)
               ...
               <div class="column w-full g-5">
                ...
                <b class="m-left-auto">{{ ucfirst($data->json->account_name ?? 'automatic') }}</b>
               </div>
            </div>
              <div class="grid grid-2 g-10 break-word">
            ...
            <strong class="desc m-left-auto c-green">&#8358;{{ number_format($data->amount,2) }}</strong>
               ...
               <div class="column w-full g-5">
                ...
                <b class="m-left-auto">{{ ucfirst($data->json->account_name ?? '') }}</b>
               </div>
            </div>
              <div class="grid grid-2 g-10 break-word">
               <div class="column w-full g-5">
                ...
                <b class="m-left-auto {{ $data->class == 'credit' ? 'c-green' : 'c-red' }}">{{ ucfirst($data->class) }}</b>
               </div>
            </div>
            <strong class="desc m-left-auto c-green">&#8358;{{ number_format($data->amount,2) }}</strong>
                    @break
           ...
           @if ($data->status == 'pending')
            ...
            <div class="row space-between w-full g-10">
                 <button onclick="GetRequest(event,'{{ url()->to('admins/get/transactions/popup/render?action=approve&id='.$data->id.'') }}',this,MyFunc.CallBack)" class="btn-green m-left-auto">
                    Approve
                </button>
                <button  onclick="GetRequest(event,'{{ url()->to('admins/get/transactions/popup/render?action=reject&id='.$data->id.'') }}',this,MyFunc.CallBack)" class="btn-red">
                    Reject
                </button>
            </div>
           @endif
